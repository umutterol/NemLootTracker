...
function WishListTracker_UI:CreateMainFrame(items, specName)
    ...
    for idx, slotKey in ipairs(enchSlotOrder) do
        ...
        -- Enchant name and popularity (with tooltip and chat link)
        local enchant = items.enchants and items.enchants[slotKey]
        if enchant then
            local enchName = CreateFrame("Button", nil, slotFrame)
            enchName:SetPoint("TOPLEFT", slotFrame, "TOPLEFT", 0, -20)
            enchName:SetWidth(enchColWidth - 80)
            ...
            popText:SetPoint("TOPRIGHT", slotFrame, "TOPRIGHT", -8, -20)
            popText:SetTextColor(1, 1, 1)
    ...
    for i, gem in ipairs(items.epic_gems or {}) do
        ...
        gemName:SetPoint("LEFT", gemFrame, "LEFT", 8, 0)
        gemName:SetWidth(enchColWidth - 80)
